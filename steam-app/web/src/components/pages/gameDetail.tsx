  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/games/${gameId}/recommendations/developer`, {
        ...
        .then((r) => r.json())
        .catch(() => {}),
      fetch(`/api/games/${gameId}/recommendations/similar`, {
        signal: c.signal,
      })
        .then((r) => r.json())
        .catch(() => {}),
    ])
      .then(([genreRecs, devRecs, similarRecs]) => {
        setSameGenreGames(Array.isArray(genreRecs) ? genreRecs : [])
        setDeveloperGames(Array.isArray(devRecs) ? devRecs : [])
        setSimilarGames(Array.isArray(similarRecs) ? similarRecs : [])
      })
      .catch(() => {})
        ...
        <div style={{ marginTop: 48 }}>
          ...
          <div>
            <h3 style={{ fontSize: 20, fontWeight: 600, marginBottom: 16 }}>
              ...
              >
                (Based on developers, genre, description & score)
              </span>
            </h3>
            <div>
              {recsLoading ? (
                ...
                >
                  <CarouselContent>
                    {similarGames.slice(0, 10).map((game: any) => (
                      <CarouselItem
                        key={`${game.id}-similar`}
                        className="md:basis-1/3 lg:basis-1/4"
                      >
                              ...
                              >
                                {game.shared_devs > 0 && (
                                  ...
                                  >
                                    ...
                                    />
                                    {game.shared_devs} dev
                                    {game.shared_devs > 1 ? 's' : ''}
                                  </Badge>
                                )}
                                {game.same_primary_genre === 1 && (
                                  <Badge
                                    variant="secondary"
                                    style={{ fontSize: 10, padding: '2px 6px' }}
                                  >
